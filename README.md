...
| "El Puente"  | Created a dashboard for Consulting Firms where their corporate clients can chat with Gemini and search through a vector database containing developers that could best fit their project requirements. Built for Build with AI Hackathon.                                | Python + FastAPI to build the API. Next.js for develping the dashboard. GeminiAPI as chatbot and embeddings function. Qdrant as vector database to store developers information, which can be later easily retrieved using Gemini (RAG technique). | [gdg-ai](https://github.com/brauliorivas/gdg-ai/)             |
